...
import { MessageCircle, Plus, Trash2, Camera, FileText, Newspaper, Image as ImageIcon } from 'lucide-react'

const WHATSAPP_NUMERO = '56958540928'
const LINK_TERMINOS = 'https://docs.google.com/forms/d/e/1FAIpQLSfUWWCQPOlvTI5a7tVhAqOti3aYzLIp7N2Np9wAubf5pgxFHQ/viewform'

export default function General() {
  const { usuarioActual, noticias, fotosGym, crearNoticia, eliminarNoticia, subirFotoGym, eliminarFotoGym } = useAuth()
  ...
  function handleFoto(e) {
    ...
    reader.onload = () => subirFotoGym(reader.result)
  ...
  return (
    <div className="min-h-screen bg-ink pb-24 px-6 pt-6">
      <p className="font-display text-3xl text-white leading-tight">General</p>
      <p className="text-white/40 text-xs mt-1 mb-6">Novedades del gimnasio</p>

      {/* Muro de noticias */}
      <div className="flex items-center justify-between mb-3">
        <p className="text-white/40 text-xs uppercase tracking-wide">Muro de noticias</p>
        {esAdmin && (
          <button
      ...
      {mostrarForm && (
        <form onSubmit={handleCrearNoticia} className="bg-white/[0.04] border border-white/10 rounded-2xl p-4 mb-3 flex flex-col gap-2">
          <input
            value={form.titulo}
          ...
          />
          <button type="submit" className="bg-cyan-brand text-ink font-semibold rounded-xl py-2.5 text-sm transition-transform active:scale-[0.98]">
            Publicar
          </button>
        </form>
      )}
      ...
      {ultimaNoticia && (
        <div className="relative bg-white/[0.04] border border-cyan-brand/25 rounded-3xl p-5 mb-3 overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-2 mb-1">
            <p className="text-cyan-brand text-[10px] font-semibold tracking-[0.2em] uppercase">Última publicación</p>
            {esAdmin && (
              <button onClick={() => eliminarNoticia(ultimaNoticia.id)} className="text-red-400/60 shrink-0 hover:text-red-400 transition-colors">
                <Trash2 size={14} />
              </button>
            )}
          </div>
          <p className="text-white font-display text-xl leading-tight mt-2">{ultimaNoticia.titulo}</p>
          <p className="text-white/60 text-sm mt-1.5">{ultimaNoticia.contenido}</p>
          <p className="text-white/30 text-xs mt-3">{formatearFecha(ultimaNoticia.created_at)}</p>
        </div>
      )}
      ...
      {restoNoticias.length > 0 && (
        <div className="flex flex-col gap-2 mb-8">
          {restoNoticias.map((n) => (
            <div key={n.id} className="bg-white/[0.04] border border-white/10 rounded-2xl p-4">
              <div className="flex items-start justify-between gap-2">
                <p className="text-white font-display text-lg leading-tight">{n.titulo}</p>
                {esAdmin && (
                  <button onClick={() => eliminarNoticia(n.id)} className="text-red-400/60 shrink-0 hover:text-red-400 transition-colors">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
              <p className="text-white/60 text-sm mt-1">{n.contenido}</p>
              <p className="text-white/25 text-xs mt-2">{formatearFecha(n.created_at)}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Fotos del gym */}
      <div className="flex items-center justify-between mb-3">
        <p className="text-white/40 text-xs uppercase tracking-wide">Fotos del gimnasio</p>
        {esAdmin && (
      ...
      {fotosGym.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-2 mb-8">
          {fotosGym.map((f) => (
            <div key={f.id} className="relative aspect-square rounded-xl overflow-hidden bg-white/5 border border-white/10">
              <img src={f.url} alt="Foto del gimnasio" className="w-full h-full object-cover" />
              {esAdmin && (
                <button
      ...
      {/* Dudas y consultas */}
      <p className="text-white/40 text-xs uppercase tracking-wide mb-3">Dudas y consultas</p>
      ...
      >
        ...
        <div>
          ...
          <p className="text-white/50 text-xs">Escríbenos directo por WhatsApp</p>
        </div>
      </a>
      ...
      >
        ...
        <div>
          <p className="text-white text-sm font-medium">Términos y condiciones</p>
          <p className="text-white/50 text-xs">Revisa las condiciones de tu membresía</p>
        </div>
      </a>
    </div>
  )
